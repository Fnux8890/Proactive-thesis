            ...
            if col in df.columns:
                if df[col].dtype == 'object':
                    ...
                    if is_01:
                        ...
                        print(f"  Warning: Numeric column '{col}' is not strictly 0/1. Skipping direct 0/1 conversion. Dtype: {df[col].dtype}")
                else:
                    print(f"  Warning: Column '{col}' intended for bool->int conversion has unhandled dtype: {df[col].dtype}. Skipping conversion.")
            else:
...
def resample_data_for_era(df: pd.DataFrame, era_identifier: str, era_config: dict, common_config: dict) -> pd.DataFrame:
    ...
    if isinstance(current_df.index, pd.DatetimeIndex) and current_df.index.name == time_col:
        ...
        if not pd.api.types.is_datetime64_any_dtype(current_df[time_col]):
            ...
            if current_df.empty:
                print(f"  (Era: {era_identifier}) DataFrame became empty after failed time conversion. Cannot resample.")
                return current_df
        ...
        print(f"  Error (Era: {era_identifier}): Time column '{time_col}' not found. Cannot resample.")
        return df

    if not isinstance(current_df.index, pd.DatetimeIndex):
        print(f"  Error (Era: {era_identifier}): Index is not DatetimeIndex before resample. Resampling may fail.")
        return df
