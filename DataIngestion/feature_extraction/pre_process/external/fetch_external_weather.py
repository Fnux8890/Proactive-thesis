...
def create_weather_table_if_not_exists(engine, table_name: str):
    ...
    try:
        ...
        print(
            f"Table '{table_name}' ensured to exist with up-to-date columns based on HOURLY_WEATHER_VARS."
        )
        return True
...
def fetch_open_meteo_data(
    start_date: str, end_date: str, latitude: float, longitude: float, hourly_vars: list
) -> pd.DataFrame:
    ...
    print(
        f"Fetching weather data from {start_date} to {end_date} for lat={latitude}, lon={longitude} with variables: {hourly_vars}"
    )
    ...
    for attempt in range(max_retries):
        try:
            response = requests.get(
                OPEN_METEO_ARCHIVE_API_URL, params=params, timeout=60
            )  # Increased timeout
            ...
            if "hourly" not in data or "time" not in data["hourly"]:
                print(
                    f"Warning: 'hourly' data or 'time' not found in API response for {start_date}-{end_date}. Skipping."
                )
            ...
            if attempt < max_retries - 1:
                ...
                print(
                    f"Failed to fetch data after {max_retries} attempts for period {start_date}-{end_date}."
                )
            ...
            if attempt < max_retries - 1:
                ...
                print(
                    f"Failed to decode JSON after {max_retries} attempts for period {start_date}-{end_date}."
                )
                return pd.DataFrame()
...
def main():
    ...
    try:
        ...
        if table_exists_check:
            print(
                f"Fetching all historical data from {HISTORICAL_START_DATE} to {HISTORICAL_END_DATE}."
            )
            ...
            if not weather_df.empty:
                ...
                report_data.append(
                    ("Basic Stats of Fetched Data (Describe)", weather_df_for_report.describe())
                )
            else:
            ...
            print(
                f"Critical Error: Table '{DB_TABLE_NAME}' does not exist and could not be verified after creation attempt. Cannot proceed."
            )
            report_data.append(
                (
                    "DB Table Status",
