...
def run_sql_script(engine, script_path: "Path") -> bool: # Use "Path" for type hint if Path is not imported here
...
def fetch_and_prepare_external_weather_for_era(era_start_date_str: str, era_end_date_str: str, target_frequency: str, time_col_name: str, engine) -> pd.DataFrame:
    ...
    try:
        test_existence_query = text(f"SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'external_weather_aarhus');")
        with engine.connect() as conn_debug:
            table_actually_exists = conn_debug.execute(test_existence_query).scalar()
            print(f"DEBUG (fetch_weather in database_operations.py): Table 'external_weather_aarhus' exists according to this engine? {table_actually_exists}")
