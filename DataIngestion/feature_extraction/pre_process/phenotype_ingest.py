...
from db_utils import SQLAlchemyPostgresConnector # Assuming db_utils.py is in the same directory or PYTHONPATH
from typing import List, Dict, Any, Iterable, Iterator # Added Iterable, Iterator
...
def convert_to_hypertable_optional(engine, table_name: str, time_column_name: str = "data_extraction_date"):
    """
    ...
    try:
        ...
        print(f"Table '{table_name}' successfully converted/confirmed as a hypertable on column '{time_column_name}'.")
    except Exception as e:
        print(f"Warning: Could not convert '{table_name}' to hypertable (or check status): {e}")
        print("This might be okay if TimescaleDB specific features are not strictly needed for this table, or if the extension is not enabled.")
...
def load_json_data_stream(json_path: Path) -> Iterator[Dict[str, Any]]:
    ...
    try:
        with open(json_path, 'r', encoding='utf-8') as f: # Use text mode for json.load
            data = json.load(f)
        ...
        print(f"Successfully loaded {len(phenotype_records)} records from {json_path} using json.load().")
...
def insert_phenotype_data_bulk(engine, data_stream: Iterable[Dict[str, Any]], table_name: str, batch_size: int = INSERT_BATCH_SIZE):
    """
    ...
    column_names = [
        "entry_id", "publication_source", "species", "cultivar_or_line", 
        "experiment_description", "phenotype_name", "phenotype_value", 
        "phenotype_unit", "measurement_condition_notes", "control_group_identifier",
        "environment_temp_day_C", "environment_temp_night_C", "environment_photoperiod_h",
        "environment_DLI_mol_m2_d", "environment_light_intensity_umol_m2_s",
        "environment_light_quality_description", "environment_CO2_ppm", "environment_RH_percent",
        "environment_stressor_type", "environment_stressor_level", 
        "data_extraction_date", "additional_notes"
    ]
    ...
    with engine.connect() as connection: # A single connection for all batches
        for record in data_stream:
            ...
            if record.get('entry_id') is None:
                print(f"Skipping record (stream index approx {total_processed_records_from_stream-1}) due to missing 'entry_id'.")
            ...
            for col_name in column_names:
                ...
                if params[col_name] is None and col_name in ["phenotype_value", "environment_temp_day_C", "environment_temp_night_C",
                                    "environment_photoperiod_h", "environment_DLI_mol_m2_d", 
                                    "environment_light_intensity_umol_m2_s", "environment_CO2_ppm", 
                                    "environment_RH_percent"]:
                    params[col_name] = None # Explicitly set to SQL NULL if missing and nullable numeric
            ...
            if len(batch_params) >= batch_size:
                ...
                try:
                    ...
                    with connection.begin(): 
                        result = connection.execute(text(stmt_template_final), batch_params)
                    ...
                    print(f"Processed batch of {len(batch_params)}. DB rows attempted in batch: {current_batch_rowcount}")
                except sqlalchemy_exc.IntegrityError as ie:
                    print(f"IntegrityError during a batch insert (first entry_id: {batch_params[0].get('entry_id','N/A')}). Batch skipped. Error: {ie}")
                except Exception as e:
                    print(f"Error during a batch insert (first entry_id: {batch_params[0].get('entry_id','N/A')}): {e}. Batch skipped.")
                finally:
        ...
        if batch_params: # Process the last batch
            ...
            try:
                ...
                with connection.begin():
                    ...
                    total_rows_attempted_in_db += len(batch_params)
                    current_batch_rowcount = len(batch_params) # Rows attempted in this batch
                print(f"Processed final batch of {len(batch_params)}. DB rows attempted in batch: {current_batch_rowcount}")
            except sqlalchemy_exc.IntegrityError as ie:
                print(f"IntegrityError during final batch insert (first entry_id: {batch_params[0].get('entry_id','N/A')}). Batch skipped. Error: {ie}")
            except Exception as e:
                print(f"Error during final batch insert (first entry_id: {batch_params[0].get('entry_id','N/A')}): {e}. Batch skipped.")
            finally:
                total_batches_processed +=1

    print(f"Bulk data insertion attempt finished.")
    print(f"  Total records processed from stream: {total_processed_records_from_stream}")
    print(f"  Records skipped due to missing 'entry_id' before batching: {total_skipped_due_to_missing_pk}")
    ...
    print("  Note: Exact count of newly inserted vs. skipped-by-conflict rows is best verified by querying the DB.")
...
if __name__ == "__main__":
    ...
    if db_connector.engine:
        ...
        if phenotype_data_stream:
            insert_phenotype_data_bulk(db_connector.engine, phenotype_data_stream, PHENOTYPE_TABLE_NAME)
        else:
        ...
        print("Failed to create database engine. Please check connection details and database server.")
