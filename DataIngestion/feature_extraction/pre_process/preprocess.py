...
from sqlalchemy import (
    create_engine,
)  # Keep create_engine and text if used directly for engine creation

# Internal utilities - updated imports for new structure
from core import (
    DataSegmenter,
    ImputationHandler,
    OutlierHandler,
)
from utils.data_enrichment_utils import (
    EraFeatureGenerator,
    calculate_daily_weekly_aggregates,
    generate_proxy_phenotype_labels,
    load_literature_phenotypes,
    scale_data_for_era,
    synthesize_light_data,
)
...
from utils.database_operations_hybrid import save_to_timescaledb_hybrid
...
if __name__ == "__main__":
    ...
    try:
        with open(CONFIG_PATH) as f_temp_cfg:
    ...
    if app_config_for_subprocess_env:
        db_conn_settings_for_subprocess = app_config_for_subprocess_env.get(
            "database_connection", {}
        )
        subprocess_env = os.environ.copy()
        subprocess_env["DB_USER"] = db_conn_settings_for_subprocess.get("user", DB_USER_ENV)
        subprocess_env["DB_PASSWORD"] = db_conn_settings_for_subprocess.get(
        ...
        for script_name, script_path_val in scripts_to_run.items():
            ...
            if script_path_val.exists():
                try:
                    ...
                    print(
                        f"Output from {script_name}:\nSTDOUT:\n{process.stdout}\nSTDERR:\n{process.stderr}"
                    )
                    if process.returncode != 0:
                        print(f"Warning: {script_name} exited with code {process.returncode}")
                except Exception as e:
                    print(
                        f"An unexpected error occurred while trying to run {script_path_val}: {e}"
                    )
                    traceback.print_exc()
            else:
                print(
                    f"Warning: Script not found at {script_path_val}. Skipping execution of {script_name}."
                )
    else:
        print(
            "Warning: Subprocess environment not set up due to config load issue. Skipping external script runs."
        )
    ...
    if extra_cfg_path_str:
        ...
        if extra_cfg_path.exists():
            print(
                f"Found EXTRA_ERA_CONFIG: {extra_cfg_path}. Attempting to merge auto era definitions."
            )
            try:
                with open(extra_cfg_path) as f_extra_cfg:
                ...
                if auto_era_definitions and isinstance(auto_era_definitions, dict):
                    if "era_definitions" not in app_config:
                        app_config["era_definitions"] = {}
                    elif not isinstance(app_config["era_definitions"], dict):
                        print(
                            "Warning: 'era_definitions' in main config is not a dict, overwriting with empty dict before merge."
                        )
                        app_config["era_definitions"] = {}
                    ...
                    print(
                        f"Warning: 'auto_era_definitions' key not found or not a dict in {extra_cfg_path}."
                    )
    ...
    if single_era_env:
        if single_era_env in eras_to_process_config:
            ...
            print(
                f"Warning: PROCESS_ERA_IDENTIFIER '{single_era_env}' not found in era_definitions. Processing all defined eras."
            )
    ...
    for era_id in eras_to_run_keys:
        ...
        if source_df.empty:
            print(
                f"No source data fetched for Era '{era_id}'. Skipping further processing for this era."
            )
            generate_summary_report(
                current_era_summary_items,
                OUTPUT_DATA_DIR,
                SUMMARY_REPORT_FILENAME_TEMPLATE.format(era_identifier=era_id),
            )
            continue

        prepared_df = sort_and_prepare_df(source_df, app_config, era_identifier=era_id)
        current_era_summary_items.append(
            ("Prepared Data Shape (after sort, ID, bool_to_int)", prepared_df.shape)
        )
        ...
        if time_col_name_common in prepared_df.columns:
            if not pd.api.types.is_datetime64_any_dtype(prepared_df[time_col_name_common]):
                prepared_df[time_col_name_common] = pd.to_datetime(
                    prepared_df[time_col_name_common], errors="coerce", utc=True
                )
            ...
            print(
                f"Warning: Time column '{time_col_name_common}' not found as column or index in prepared_df for Era '{era_id}'. Subsequent steps might fail."
            )
        ...
        era_target_freq = era_conf_details.get(
            "target_frequency", common_settings_cfg.get("default_target_frequency", "15T")
        )
        ...
        if common_settings_cfg.get("include_external_weather", False):
            ...
            if era_start_date_str and era_end_date_str:
                external_weather_df = fetch_and_prepare_external_weather_for_era(
                    ...
                    era_end_date_str,
                    era_target_freq,
                    time_col_name_common,
                    engine,
                )
                if not external_weather_df.empty:
                    if df_with_weather.index.tz is None:
                        df_with_weather.index = df_with_weather.index.tz_localize("UTC")
                    if external_weather_df.index.tz is None:
                        external_weather_df.index = external_weather_df.index.tz_localize("UTC")

                    df_with_weather = df_with_weather.join(
                        external_weather_df, how="left", rsuffix="_ext_weather"
                    )
                    print(
                        f"Shape after attempting to join external weather: {df_with_weather.shape}"
                    )
                    current_era_summary_items.append(
                        (
                            "External Weather Data Merged",
                            bool(not external_weather_df.empty),
                        )
                    )
                    current_era_summary_items.append(
                        ("Shape after weather merge", df_with_weather.shape)
                    )
                else:
                ...
                current_era_summary_items.append(
                    ("External Weather Data Fetch Skipped", "Missing era start/end dates")
                )
        else:
        ...
        if common_settings_cfg.get("include_energy_prices", False):
            ...
            if era_start_date_str and era_end_date_str:
                ...
                if not energy_prices_df.empty:
                    if df_final_merged.index.tz is None:
                        df_final_merged.index = df_final_merged.index.tz_localize("UTC")
                    if energy_prices_df.index.tz is None:
                        energy_prices_df.index = energy_prices_df.index.tz_localize("UTC")

                    df_final_merged = df_final_merged.join(
                        energy_prices_df, how="left", rsuffix="_energy_price"
                    )
                    output_spot_price_col_name = "spot_price_dkk_mwh"

                    cols_to_ffill_energy = [
                        col for col in df_final_merged.columns if output_spot_price_col_name in col
                    ]
                    if cols_to_ffill_energy:
                        print(
                            f"Forward filling merged energy price columns: {cols_to_ffill_energy}"
                        )
                        df_final_merged[cols_to_ffill_energy] = df_final_merged[
                            cols_to_ffill_energy
                        ].ffill()

                    print(f"Shape after attempting to join energy prices: {df_final_merged.shape}")
                    current_era_summary_items.append(
                        ("Energy Price Data Merged", bool(not energy_prices_df.empty))
                    )
                    current_era_summary_items.append(
                        ("Shape after energy price merge", df_final_merged.shape)
                    )
                else:
                ...
                current_era_summary_items.append(
                    ("Energy Price Data Fetch Skipped", "Missing era start/end dates")
                )
        else:
        ...
        df_resampled = resample_data_for_era(
            df_final_merged, era_id, era_conf_details, common_settings_cfg
        )
        ...
        if common_settings_cfg.get("enable_light_synthesis", False):
            ...
            if df_enriched.empty:
                print(
                    f"  Skipping light synthesis for Era '{era_id}': input DataFrame is empty before synthesis."
                )
                current_era_summary_items.append(
                    ("Light Synthesis Attempted", "Skipped, empty input DF")
                )
            else:
                try:
                    ...
                    if df_synthesized is not None and not df_synthesized.empty:
                        ...
                        print(
                            f"  Successfully synthesized light data. DataFrame shape after synthesis: {df_enriched.shape}"
                        )
                        current_era_summary_items.append(("Light Synthesis Status", "Success"))
                        if "par_synth_umol" in df_enriched.columns:
                            current_era_summary_items.append(
                                ("Synthetic PAR Column", "par_synth_umol added")
                            )
                        else:
                            current_era_summary_items.append(
                                ("Synthetic PAR Column", "par_synth_umol NOT added")
                            )
                    else:
                        print(
                            f"  Light synthesis for Era '{era_id}' resulted in an empty or None DataFrame. Original data retained."
                        )
                        current_era_summary_items.append(
                            ("Light Synthesis Status", "Failed - empty/None result")
                        )
                except Exception as e:
                    print(f"  Error during light synthesis for Era '{era_id}': {e}")
                    traceback.print_exc()
                    current_era_summary_items.append(
                        ("Light Synthesis Status", f"Failed - Exception: {type(e).__name__}")
                    )
        else:
        ...
        if proxy_phenotype_config and not literature_phenotypes_df.empty:
            ...
            for pheno_target_info in proxy_phenotype_config.get("targets", []):
                ...
                if not proxy_series.empty:
                    ...
                    current_era_summary_items.append(
                        (f"Proxy Phenotype Added: {proxy_series.name}", True)
                    )
                    print(f"  Added proxy phenotype: {proxy_series.name}")
                else:
                    current_era_summary_items.append(
                        (
                            f"Proxy Phenotype Skipped: {pheno_target_info.get('name')}",
                            "No data or error",
                        )
                    )
        ...
        if common_settings_cfg.get("calculate_time_aggregates", False):
            ...
            df_daily_agg, df_weekly_agg = calculate_daily_weekly_aggregates(
                df_enriched, era_id
            )  # This function is now imported
        ...
        outlier_handler = OutlierHandler(
            outlier_rules, rules_cfg_dict=common_settings_cfg.get("outlier_configs", {})
        )
        ...
        segmenter = DataSegmenter(
            era_conf_details, common_config=app_config.get("common_settings", {})
        )
        if (
            df_after_outliers.index.name != time_col_name_common
            and time_col_name_common in df_after_outliers.columns
        ):
            if not pd.api.types.is_datetime64_any_dtype(df_after_outliers[time_col_name_common]):
                df_after_outliers[time_col_name_common] = pd.to_datetime(
                    df_after_outliers[time_col_name_common], errors="coerce", utc=True
                )
            ...
            not isinstance(df_after_outliers.index, pd.DatetimeIndex)
            and df_after_outliers.index.name == time_col_name_common
        ):
        ...
        if not data_segments:
            print(
                f"No data segments found for Era '{era_id}'. Skipping further processing for this era."
            )
            generate_summary_report(
                current_era_summary_items,
                OUTPUT_DATA_DIR,
        ...
        for i, segment_df_original in enumerate(data_segments):
            ...
            current_era_summary_items.append(
                (f"Segment {i + 1} Initial Shape", segment_df_original.shape)
            )
            ...
            era_imputation_rules_ref = era_conf_details.get(
                "imputation_rules_ref", "default_imputation_rules"
            )
            imputation_rules = app_config.get("preprocessing_rules", {}).get(
                era_imputation_rules_ref, []
            )
            ...
            current_era_summary_items.append(
                (f"Segment {i + 1} Shape after Imputation", df_after_imputation.shape)
            )

            df_with_lamp_features = df_after_imputation.copy()
            lamp_cols = [
                col
                for col in df_with_lamp_features.columns
                if "lamp" in col.lower() and "status" in col.lower()
            ]
            if lamp_cols:
                try:
                    for lamp_col in lamp_cols:
                        df_with_lamp_features[lamp_col] = pd.to_numeric(
                            df_with_lamp_features[lamp_col], errors="coerce"
                        ).fillna(0)

                    df_with_lamp_features["total_lamps_on"] = df_with_lamp_features[lamp_cols].sum(
                        axis=1
                    )
                    print(f"  Calculated 'total_lamps_on' from columns: {lamp_cols}")
                    current_era_summary_items.append(
                        (f"Segment {i + 1} Total Lamps Feature Added", True)
                    )
                except Exception as e_lamp:
                    print(f"  Error calculating total_lamps_on: {e_lamp}")
                    current_era_summary_items.append(
                        (f"Segment {i + 1} Total Lamps Feature Error", str(e_lamp))
                    )
            else:
                current_era_summary_items.append(
                    (f"Segment {i + 1} Total Lamps Feature Added", "No lamp status cols found")
                )

            df_processed_segment = df_with_lamp_features
            skip_era_feature_env = os.getenv("SKIP_ERA_FEATURE", "false").lower()
            should_skip_era_feature_generation = (
                skip_era_feature_env == "true" or skip_era_feature_env == "1"
            )

            if not should_skip_era_feature_generation:
                default_era_c_label_file = f"{era_id}_era_labels_levelC.parquet"
                era_feature_file_name = era_conf_details.get(
                    "era_feature_file",
                    app_config.get("common_settings", {}).get(
                        "era_feature_file", default_era_c_label_file
                    ),
                )
                ...
                print(
                    f"Attempting to add era features using file: {era_feature_file_path} for {segment_label}"
                )
                ...
                current_era_summary_items.append(
                    (f"Segment {i + 1} Era Feature Gen", shape_change_info)
                )
            else:
            ...
            df_scaled, _ = scale_data_for_era(
                df_processed_segment,
                era_id,
                era_conf_details,
                app_config,
                SCALERS_DIR,
            ...
            current_era_summary_items.append(
                (f"Segment {i + 1} Shape after Scaling", df_scaled.shape)
            )
            ...
            output_filename = OUTPUT_FILENAME_TEMPLATE.format(
                era_identifier=era_id, segment_num=i + 1
            )
            ...
            if engine and verify_table_exists(engine, "preprocessed_features"):
                ...
                save_to_timescaledb_hybrid(
                    df=df_scaled,
                    era_identifier=era_id,
                    db_config=db_config_for_hybrid,
                    table_name="preprocessed_features",
                )
                print(
                    f"INFO: Attempted save for era {era_id}, segment {i + 1} to 'preprocessed_features' using hybrid method."
                )
            elif engine:  # If engine exists but table doesn't
                print(
                    f"WARNING: Table 'preprocessed_features' does not exist. Skipping save for era {era_id}, segment {i + 1}. Ensure the table is created with the correct schema."
                )
            else:  # If engine itself is not available
                print(
                    f"CRITICAL: Database engine not available. Skipping save for era {era_id}, segment {i + 1} to hybrid table."
                )
