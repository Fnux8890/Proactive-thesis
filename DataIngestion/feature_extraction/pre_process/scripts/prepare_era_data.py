...
def get_db_connection_details() -> dict:
    ...
    if missing_details:
        raise ValueError(
            f"Missing database connection environment variables for: {missing_details}. Please set them."
        )
    return details
...
def load_era_data(era_identifier: str, config: dict) -> pd.DataFrame:
    ...
    if not all([db_table, start_date, end_date, time_column]):
        print(
            f"Error: Missing db_table, start_date, end_date, or time_column in config for era '{era_identifier}'."
        )
        return pd.DataFrame()
    ...
    try:
        ...
        print(
            f"Successfully connected to database '{conn_details['dbname']}' on host '{conn_details['host']}'."
        )
...
def initial_clean_era_data(df: pd.DataFrame, era_identifier: str, config: dict) -> pd.DataFrame:
    ...
    time_column = config.get("common_settings", {}).get("time_column_name", "time")
    dead_columns_era = (
        config.get("era_specific_settings", {})
        .get(era_identifier, {})
        .get("dead_columns_to_drop", [])
    )
...
def resample_era_data(df: pd.DataFrame, era_identifier: str, config: dict) -> pd.DataFrame:
    ...
    if time_column not in df.columns:
        print(
            f"Error during resampling: Time column '{time_column}' not found. Skipping resampling."
        )
        return df
    if not pd.api.types.is_datetime64_any_dtype(df[time_column]):
        print(
            f"Error during resampling: Time column '{time_column}' is not datetime. Skipping resampling."
        )
        return df
    ...
    try:
        ...
        df_resampled = df.resample(
            target_freq
        ).asfreq()  # Use .asfreq() to just conform to new freq, NaN for new slots
...
def impute_era_data(df: pd.DataFrame, era_identifier: str, config: dict) -> pd.DataFrame:
    ...
    imputation_config = (
        config.get("era_specific_settings", {}).get(era_identifier, {}).get("imputation", {})
    )
    ...
    print(
        f"Applying imputation strategy (default: method='{default_method}', limit={default_limit})..."
    )
    ...
    if default_method == "ffill":
        ...
        print(
            f"Warning: Unknown default imputation method '{default_method}'. No imputation applied by default."
        )
...
def main():
    ...
    era_to_process = config.get("common_settings", {}).get(
        "default_era_to_process_for_script", "Era1"
    )
