...
def fetch_energi_data_service_prices(start_date: str, end_date: str, price_areas: List[str], dataset_name: str) -> pd.DataFrame:
    ...
    print(f"Fetching energy prices from {start_date} to {end_date} for areas {price_areas} from dataset {dataset_name}.")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not isinstance(data, dict) or 'records' not in data or not isinstance(data['records'], list):
                print(f"Warning: 'records' key not found, not a list, or main response is not a dict for {start_date}-{end_date}, areas {price_areas}.")
                print(f"Response JSON: {json.dumps(data, indent=2)[:1000]}") # Log part of the response
                return pd.DataFrame()
            
            records = data['records']
            if not records: # Handles case where 'records' is an empty list
                print(f"Warning: No 'records' found in API response for {start_date}-{end_date}, areas {price_areas}.")
                print(f"Response JSON: {data}") # Log the full response for debugging
                return pd.DataFrame()

            df = pd.DataFrame(records)
            print(f"Successfully fetched {len(df)} records for {start_date}-{end_date}, areas {price_areas}.")
            
            # Data Cleaning and Type Conversion
            if API_TIME_COLUMN not in df.columns:
                print(f"Error: Expected time column '{API_TIME_COLUMN}' not in fetched data. Columns: {df.columns}")
                return pd.DataFrame()
            ...
            try:
                ...
                if len(df) < original_len:
                    print(f"  Dropped {original_len - len(df)} rows due to NaT in time column after DST handling.")

            except Exception as e:
                print(f"Error converting time column '{API_TIME_COLUMN}': {e}. Skipping this batch.")
                return pd.DataFrame()

            # Ensure price column is numeric
            if API_SPOT_PRICE_COLUMN in df.columns:
                df[API_SPOT_PRICE_COLUMN] = pd.to_numeric(df[API_SPOT_PRICE_COLUMN], errors='coerce')
            else:
...
def save_data_to_db(df: pd.DataFrame, engine, table_name: str) -> bool:
    ...
    try:
        ...
        print("This might be due to duplicate primary keys if data for this period/area already exists.")
        return False
    except Exception as e:
    ...
    try:
        ...
        if table_exists_check:
            ...
            if not energy_df.empty:
                ...
                report_data.append(("Basic Stats of Fetched Data (Describe)", energy_df_for_report.describe()))
            else:
                print("No energy price data fetched. Database will not be updated.")
                report_data.append(("Data Fetching Outcome", "No data fetched."))
        elif engine:
            print(f"Critical Error: Table '{DB_TABLE_NAME}' does not exist and could not be verified. Cannot proceed.")
