...
def _ensure_preprocessed_features_table_exists(cur, table_name: str):
    ...
    if table_exists_initially:
        ...
        existing_columns = (
            set(existing_columns_row[0])
            if existing_columns_row and existing_columns_row[0]
            else set()
        )

        # Heuristic for old schema: 'features' column exists AND 'extended_features' does not.
        is_old_schema = (
            "features" in existing_columns and "extended_features" not in existing_columns
        )

        if is_old_schema:
            logger.warning(
                f"Table {table_name} found with old schema (contains 'features' column, missing 'extended_features'). Dropping for recreation."
            )
            cur.execute(
                f"DROP TABLE public.{table_name};"
            )  # Use f-string for table name, ensure it's safe
            ...
            if not is_hypertable:
                logger.info(
                    f"Table {table_name} exists with compatible schema but is not a hypertable. Converting..."
                )
                # For create_hypertable, table_name is passed as a parameter for psycopg2 to handle quoting
                cur.execute(
                    "SELECT create_hypertable(%s, 'time', if_not_exists => TRUE, chunk_time_interval => INTERVAL '7 days');",
                    (table_name,),
                )
                logger.info(f"Table {table_name} converted to hypertable.")
            else:
                logger.debug(
                    f"Table {table_name} is already a hypertable with a compatible schema."
                )
...
def save_to_timescaledb_hybrid(
    ...
    for row in df.iterrows():
        # Core column values
        record = [row[1]["time"], era_identifier]
    ...
    try:
        with get_db_connection(db_config) as conn, conn.cursor() as cur:
            ...
            for i in range(0, len(records), batch_size):
                ...
                conn.commit()
                logger.info(
                    f"Inserted batch {i // batch_size + 1}/{(len(records) - 1) // batch_size + 1}"
                )
    ...
    try:
        with get_db_connection(db_config) as conn, conn.cursor() as cur:
            ...
            while current_time < max_time:
                ...
                logger.info(
                    f"Migrated {rows_migrated} rows for period {current_time} to {next_time}"
                )
