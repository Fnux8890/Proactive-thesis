...
pub fn parse_csv(config: &FileConfig, file_path: &Path) -> Result<Vec<ParsedRecord>, ParseError> {
    // println!("DEBUG: Entering parse_csv for {}", file_path.display()); // <-- Comment out
    ...
    if has_headers {
        ...
        if let Some(ts_info) = &config.timestamp_info {
            ...
            if let Some(name) = &ts_info.datetime_col_name {
                ...
                timestamp_strategy = TimestampStrategy::DateTimeByIndex(
                    index,
                    format_opt
                        .ok_or_else(|| missing_format_error(file_path, "datetime_col_name"))?
                        .to_string(),
                );
            } else if let (Some(date_name), Some(time_name)) =
                (&ts_info.date_col_name, &ts_info.time_col_name)
            {
                let date_index = get_index(date_name, "date_col_name")?;
                let time_index = get_index(time_name, "time_col_name")?;
                timestamp_strategy = TimestampStrategy::DateIndexTimeIndex(
                    date_index,
                    time_index,
                    format_opt
                        .ok_or_else(|| missing_format_error(file_path, "date/time_col_name"))?
                        .to_string(),
                );
            ...
            } else if let Some(idx) = ts_info.datetime_col {
                // Fallback to index-based
                timestamp_strategy = TimestampStrategy::DateTimeByIndex(
                    idx,
                    format_opt
                        .ok_or_else(|| missing_format_error(file_path, "datetime_col"))?
                        .to_string(),
                );
                ...
                timestamp_strategy = TimestampStrategy::DateIndexTimeIndex(
                    ...
                    time_idx,
                    format_opt
                        .ok_or_else(|| missing_format_error(file_path, "date/time_col"))?
                        .to_string(),
                );
            ...
            for mapping in map_config {
                ...
                let source_index = match source_identifier.parse::<usize>() {
                    ...
                    Err(_) => {
                        ...
                        *header_map.get(source_identifier).ok_or_else(|| {
                            ParseError::ConfigError {
                                path: file_path.to_path_buf(),
        ...
        if let Some(ts_info) = &config.timestamp_info {
            ...
            if ts_info.datetime_col_name.is_some()
                || ts_info.date_col_name.is_some()
                || ts_info.unix_ms_col_name.is_some()
            {
                return Err(ParseError::ConfigError {
                    path: file_path.to_path_buf(),
                    field: "timestamp_info.*_col_name".to_string(),
                    message: "Column names specified for timestamp, but file has no headers."
                        .to_string(),
                });
            }
            ...
            if let Some(idx) = ts_info.datetime_col {
                timestamp_strategy = TimestampStrategy::DateTimeByIndex(
                    ...
                    format_opt
                        .ok_or_else(|| {
                            missing_format_error(file_path, "datetime_col (no headers)")
                        })?
                        .to_string(),
                );
            } else if let (Some(date_idx), Some(time_idx)) = (ts_info.date_col, ts_info.time_col) {
                timestamp_strategy = TimestampStrategy::DateIndexTimeIndex(
                    ...
                    format_opt
                        .ok_or_else(|| {
                            missing_format_error(file_path, "date/time_col (no headers)")
                        })?
                        .to_string(),
                );
            } else if let Some(idx) = ts_info.unix_ms_col {
                ...
                let source_index =
                    source_identifier
                        ...
                        .map_err(|_| ParseError::ConfigError {
                            ...
                            field: format!("column_map.source: {}", source_identifier),
                            message:
                                "Source column must be a valid index when file has no headers."
                                    .to_string(),
                        })?;
    ...
    for i in 0..remaining_rows_to_skip {
        ...
        match reader.read_record(&mut record) {
            ...
            Ok(false) => {
                eprintln!(
                    "WARN: Reached end of file while skipping header row {}/{} in {}",
                    i + 1,
            ...
            Err(e) => {
                return Err(ParseError::IoError {
                    ...
                    source: std::io::Error::new(
                        ...
                        format!("Error skipping header row {}: {}", i + 1, e),
                    ),
                });
            }
        }
    ...
    for (row_index, result) in reader.records().enumerate() {
        let record = match result {
            ...
            Err(e) => {
                ...
                eprintln!(
                    "ERROR: Failed to read record at file row {} in {}: {}",
                    file_row_num,
                    file_path.display(),
                    e
                );
            ...
            match get_field_by_index(&record, *source_index) {
                ...
                Ok(Some(raw_value)) => {
                    ...
                    match data_type.as_str() {
                        ...
                        unknown_type => {
                            // Catch-all for truly unknown types
                ...
                Ok(None) => {
                    /* Field was empty/whitespace after trimming - already None */
                    continue;
                }
                Err(_) => {
                    // Error from get_field_by_index (should indicate index out of bounds)
                    eprintln!(
                        "ERROR: Missing column at expected index {} for field '{}' in {} at row {}",
                        *source_index,
                        target_field,
                        file_path.display(),
                        file_row_num
        ...
        if !row_has_error {
            ...
            if parsed_record.timestamp_utc.is_some() {
                ...
                eprintln!(
                    "INFO: Skipping row {} in {} due to missing or invalid timestamp.",
                    file_row_num,
                    file_path.display()
                );
            }
    ...
    // println!(
    //     "DEBUG: Finished parse_csv for {}. Parsed {} records.", // <-- Comment out
    //     file_path.display(),
    //     parsed_records.len()
    // );
    Ok(parsed_records)
...
fn parse_unix_ms_utc(ms_str: &str) -> Result<DateTime<Utc>, String> {
    ms_str
        ...
        .map_err(|e| {
            format!(
                "Failed to parse '{}' as i64 for Unix ms timestamp: {}",
                ms_str, e
            )
        })
        .and_then(|ms| {
