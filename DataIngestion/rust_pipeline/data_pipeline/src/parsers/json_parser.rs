...
pub fn parse_aarslev_stream_json(
    config: &FileConfig,
    file_path: &Path,
) -> Result<Vec<ParsedRecord>, ParseError> {
    // println!("DEBUG: Entering parse_aarslev_stream_json for {}", file_path.display()); // <-- Comment out
    ...
    // println!("DEBUG (StreamJSON): Attempting to deserialize {}", file_path.display()); // <-- Comment out
    let stream_data_vec: Vec<StreamData> =
        serde_json::from_reader(reader).map_err(|e| ParseError::JsonParseError {
            path: file_path.to_path_buf(),
            source: e,
        })?;
    // println!("DEBUG (StreamJSON): Successfully deserialized {} stream objects from {}", stream_data_vec.len(), file_path.display()); // <-- Comment out

    let mut parsed_records = Vec::new();

    // println!("DEBUG (StreamJSON): Retrieving stream_map config from file config."); // <-- Comment out
    let stream_map = config
        .stream_map
        .as_ref()
    ...
    // println!("DEBUG (StreamJSON): Iterating through {} stream objects for {}", stream_data_vec.len(), file_path.display()); // <-- Comment out
    for stream_data in stream_data_vec {
        let uuid = &stream_data.uuid;
        // println!("DEBUG (StreamJSON): Processing stream UUID: {}", uuid); // <-- Comment out
        ...
        if let Some(mapping) = stream_map.get(uuid) {
            ...
            // println!(
            //     "DEBUG (StreamJSON): Found config for UUID {}: target='{}', type='{}'",
            //     uuid, target_field, data_type
            // );

            // Iterate through the readings for this stream
            // println!(
            //     "DEBUG (StreamJSON): Iterating through {} readings for UUID {}",
            //     stream_data.readings.len(),
            //     uuid
            // );
    ...
    // println!("DEBUG: Finished parse_aarslev_stream_json for {}. Parsed {} records.", file_path.display(), parsed_records.len()); // <-- Comment out
    Ok(parsed_records)
}
