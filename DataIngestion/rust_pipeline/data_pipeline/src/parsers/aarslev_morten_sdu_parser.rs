...
macro_rules! set_field {
    ...
    ($record:expr, $field_name:expr, $value:expr; f64) => {
        match $field_name.as_str() {
            ...
            _ => eprintln!("WARN (MortenSDU): Attempted to set unhandled f64 field '{}' in ParsedRecord.", $field_name),
        }
    };
     // Match bool
     ($record:expr, $field_name:expr, $value:expr; bool) => {
        match $field_name.as_str() {
            ...
            _ => eprintln!("WARN (MortenSDU): Attempted to set unhandled bool field '{}' in ParsedRecord.", $field_name),
        }
    };
     // Fallback for unhandled types
     ($record:expr, $field_name:expr, $value:expr; $type:ty) => {
         eprintln!("WARN (MortenSDU): set_field! macro called for unhandled type '{}' for field '{}'.", stringify!($type), $field_name);
     };
}


pub fn parse_morten_sdu(config: &FileConfig, file_path: &Path) -> Result<Vec<ParsedRecord>, ParseError> {
    println!("DEBUG: Entering parse_morten_sdu for {}", file_path.display());
    ...
    let ts_info = config.timestamp_info.as_ref().ok_or_else(|| ParseError::ConfigError {
        path: file_path.to_path_buf(),
        field: "timestamp_info".to_string(),
        message: "timestamp_info is required for AarslevMortenSDU format.".to_string(),
    })?;

    let start_col_name = ts_info.start_col_name.as_deref().ok_or_else(|| ParseError::ConfigError {
        path: file_path.to_path_buf(),
        field: "timestamp_info.start_col_name".to_string(),
        message: "start_col_name is required.".to_string(),
    })?;
    let _end_col_name = ts_info.end_col_name.as_deref().ok_or_else(|| ParseError::ConfigError {
        path: file_path.to_path_buf(),
        field: "timestamp_info.end_col_name".to_string(),
        message: "end_col_name is required.".to_string(),
    })?;
    let timestamp_format = ts_info.format.as_deref().ok_or_else(|| ParseError::ConfigError {
        path: file_path.to_path_buf(),
        field: "timestamp_info.format".to_string(),
        message: "Timestamp format is required.".to_string(),
    })?;

    let start_index = *header_map.get(start_col_name).ok_or_else(|| ParseError::ConfigError {
        path: file_path.to_path_buf(),
    ...
    let mut column_index_map: Vec<(String, usize, String)> = Vec::new();
     if let Some(map_config) = &config.column_map {
        for mapping in map_config {
             ...
             let source_index = *header_map.get(source_name).ok_or_else(|| ParseError::ConfigError {
                path: file_path.to_path_buf(),
    ...
    let remaining_rows_to_skip = if header_rows_to_skip > 1 { header_rows_to_skip - 1 } else { 0 };
    for i in 0..remaining_rows_to_skip {
        let mut record = csv::StringRecord::new();
        match reader.read_record(&mut record) {
             Ok(true) => { /* Skipped */ }
             Ok(false) => {
                 eprintln!("WARN (MortenSDU): Reached end of file while skipping header row {}/{} in {}", i+1, remaining_rows_to_skip, file_path.display());
                 break;
              }
             Err(e) => return Err(ParseError::IoError { path: file_path.to_path_buf(), source: std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()) }),
         }
    }
     ...
     for (row_index, result) in reader.records().enumerate() {
        let record = match result {
            ...
            Err(e) => {
                ...
                eprintln!("ERROR (MortenSDU): Failed to read record at file row {} in {}: {}", file_row_num, file_path.display(), e);
                continue;
            }
        };
        ...
        match get_field_by_index(&record, start_index) { // Use helper
             Ok(Some(start_str)) => { // Check inner Option
                 if !null_markers.iter().any(|m| m == start_str) { // Check for null marker
             ...
             Err(e) => { // Error from get_field_by_index
        ...
        // --- Data Column Processing ---
         for (target_field, source_index, data_type) in &column_index_map {
             match get_field_by_index(&record, *source_index) { // Use helper
                 Ok(Some(raw_value)) => { // Check inner Option
                    ...
                    if *target_field == "rain_status" { // Need to dereference target_field
                    ...
                    match data_type.as_str() {
                         ...
                         "boolean" | "bool" => {
                            ...
                            match trimmed_value {
                                ...
                                "0" | "0.0" => set_field!(parsed_record, *target_field, false; bool),
                                _ => {
                 ...
                 Ok(None) => { /* Field was empty/whitespace */ continue; }
                 Err(e) => { // Error from get_field_by_index
        ...
        if !row_has_error {
            ...
            eprintln!("INFO (MortenSDU): Skipping row {} in {} due to parsing errors.", file_row_num, file_path.display());
        }
    }

    println!("DEBUG: Finished parse_morten_sdu for {}. Parsed {} records.", file_path.display(), parsed_records.len());
    Ok(parsed_records)
}

// Helper to safely get a field by index, returning Option<&str>
// Mirrors the logic in csv_parser.rs
fn get_field_by_index<'r>(record: &'r csv::StringRecord, index: usize) -> Result<Option<&'r str>, ParseError> {
    match record.get(index) {
        ...
        None => Err(ParseError::ConfigError { // Assuming missing column is a config/data mismatch
