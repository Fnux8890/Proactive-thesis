...
from ..config.config import DataConfig, GlobalConfig  # Relative import
...
def load_and_preprocess_data(data_dir: Path, cfg: DataConfig, smoke_test: bool):
    ...
    if smoke_test and len(data_to_process) < min_rows_for_test:
        raise ValueError(
            f"Insufficient data ({len(data_to_process)} rows) for smoke test with 3 splits (need ~{min_rows_for_test})."
        )
    ...
    if n_train <= 0 or n_val <= 0 or n_test <= 0:
        raise ValueError(
            f"Data too small for 70/15/15 split. Total sequences: {n_total}. Calculated splits: Train={n_train}, Val={n_val}, Test={n_test}"
        )
...
def train_model(args, cfg: GlobalConfig):
    ...
    try:
        (
            ...
            n_features,
            n_targets,
        ) = load_and_preprocess_data(data_dir, cfg.data, args.smoke_test)
    ...
    hparams_to_log = {
        ...
        "trainer": {
            k: v for k, v in cfg.trainer.__dict__.items() if k != "max_epochs"
        },  # log determined epochs separately
    ...
    model = LSTMRegressor(backbone=backbone, learning_rate=cfg.trainer.learning_rate)
    ...
    if best_model_path and Path(best_model_path).exists():
        try:
            ...
            logger.error(
                f"Failed to log checkpoint artifact {best_model_path} to MLflow: {e}",
                exc_info=True,
            )
    ...
    if scaler_path.exists():
        try:
            ...
            logger.error(
                f"Failed to log scaler artifact {scaler_path} to MLflow: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            if plot_path.exists():
                try:
                    ...
                    logger.error(
                        f"Failed to log plot artifact {plot_path} to MLflow: {e}",
                        exc_info=True,
                    )
            plt.close(fig)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override base model output directory.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=None,
        help="Explicitly set number of training epochs, overrides GPU defaults.",
    )
    parser.add_argument(
        "--smoke-test",
        action="store_true",
        help="Run a quick smoke test (1 epoch, limited data).",
    )
    ...
    if args.epochs is not None:
        ...
        config.trainer.max_epochs = config.default_epochs_by_gpu.get(
            gpu_tag, config.default_epochs_by_gpu["default_gpu"]
        )
