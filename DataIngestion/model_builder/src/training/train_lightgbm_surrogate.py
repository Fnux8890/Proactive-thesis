...
class PostgreSQLDataLoader(DataLoader):
    ...
    def __init__(self, connection_string: str, config: DataConfig):
        ...
        self._phenotypes_df: Optional[pd.DataFrame] = None
    ...
    def prepare_training_data(
        ...
        feature_cols = [col for col in features_df.columns 
                       if col not in ['era_id', 'signal_name', 'level', 'stage', 
                                     'start_time', 'end_time', 'era_rows', target_col]]
    ...
    def __init__(self, config: ModelConfig):
        ...
        if config.device == "gpu":
            try:
                ...
                logger.warning(f"GPU not available for LightGBM: {e}. Falling back to CPU.")
    ...
    def train(self, target: str, use_phenotypes: bool = False) -> Dict[str, Any]:
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            self._plot_results(
                y_test, 
                y_pred_test,
                self.model_trainer.feature_importance_,
                output_dir
            )
