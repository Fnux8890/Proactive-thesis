...
OBJECTIVES = {
    ...
    "plant_growth": OptimizationObjective(
        ...
        related_phenotypes=[PhenotypeFeature.GROWTH_RATE, PhenotypeFeature.LEAF_AREA, PhenotypeFeature.STEM_DIAMETER],
        important_signals=["dli_sum", "co2_measured_ppm", "air_temp_c", "relative_humidity_percent", "vpd_hpa"],
    ...
    "water_usage": OptimizationObjective(
        ...
        related_phenotypes=[PhenotypeFeature.WATER_USE_EFFICIENCY],
        important_signals=["relative_humidity_percent", "vpd_hpa", "radiation_w_m2", "air_temp_c"],
    ...
    "crop_quality": OptimizationObjective(
        ...
        important_signals=["dli_sum", "light_intensity_umol", "air_temp_c", "humidity_deficit_g_m3"],
    ...
    "production_time": OptimizationObjective(
        ...
        type=ObjectiveType.MINIMIZE,
        unit="days",
        related_phenotypes=[PhenotypeFeature.TIME_TO_FLOWER, PhenotypeFeature.GROWTH_RATE],
        important_signals=["dli_sum", "air_temp_c", "co2_measured_ppm"],
...
def get_objective_features(objective_name: str) -> dict[str, list[str]]:
    ...
    return {"signals": obj.important_signals, "phenotypes": [p.value for p in obj.related_phenotypes]}


def create_composite_objective(
    name: str, objectives: list[str], weights: list[float] | None = None
...
COMPOSITE_OBJECTIVES = {
    "sustainable_production": create_composite_objective(
        "sustainable_production", ["energy_consumption", "water_usage", "plant_growth"], [0.3, 0.2, 0.5]
    ),
    "quality_focused": create_composite_objective(
        "quality_focused", ["crop_quality", "climate_stability", "production_time"], [0.5, 0.3, 0.2]
    ),
    "efficient_growth": create_composite_objective(
        "efficient_growth", ["energy_consumption", "plant_growth", "production_time"], [0.3, 0.5, 0.2]
    ),
}
